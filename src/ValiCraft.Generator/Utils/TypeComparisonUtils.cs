...
public static class TypeComparisonUtils
{
    ...
    {
        if (string.IsNullOrEmpty(typeName1) || string.IsNullOrEmpty(typeName2)) return false;

        // Normalize both strings by trimming any trailing '?' character.
        if (typeName1!.EndsWith("?")) typeName1 = typeName1.Substring(0, typeName1.Length - 1);

        if (typeName2!.EndsWith("?")) typeName2 = typeName2.Substring(0, typeName2.Length - 1);

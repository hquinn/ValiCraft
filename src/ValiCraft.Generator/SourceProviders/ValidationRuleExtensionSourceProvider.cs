...
public static class ValidationRuleExtensionSourceProvider
{
        ...
        SourceProductionContext context)
    {
        foreach (var validationRuleResult in validationRuleResults)
        {
            ...
            var placeholderRemarks = GetPlaceholderRemarks(validationRule);
            ...
            var sourceCode = $$"""
                               ...
                               namespace {{validationRule.Class.Namespace}}
                               {
                                   ...
                                   {{attributesBuilder}}{{validationRule.Class.Accessibility}} static class {{validationRule.Class.Name}}Extensions
                                   {
                                       ...
                                       /// </summary>
                               {{placeholderRemarks}}        [global::{{KnownNames.Attributes.MapToValidationRule}}(typeof({{mapToValidationRuleData.FullyQualifiedUnboundedName}}), "{{mapToValidationRuleData.ValidationRuleGenericFormat}}")]
    ...
    private static string GetPlaceholderRemarks(ValidationRule validationRule)
    {
        var placeholders = new List<string> { "{TargetName}", "{TargetValue}" };
        
        foreach (var rulePlaceholder in validationRule.RulePlaceholders)
        {
            placeholders.Add(rulePlaceholder.PlaceholderName);
        }

        var placeholderList = string.Join(", ", placeholders.Select(p => $"<c>{p}</c>"));
        
        return $"""
                        /// <remarks>
                        /// Available message placeholders: {placeholderList}.
                        /// </remarks>
